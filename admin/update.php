...
<div class="row g-4">
  <div class="col-lg-7">
    <div class="card shadow-sm"><div class="card-body p-4">
      ...
      <hr>
      <?php if ($release): ?>
        <div class="text-body-secondary small"><?=current_language()==='en'?'Latest GitHub release':(current_language()==='fr'?'Dernière release GitHub':'Neuestes GitHub-Release')?></div><div class="fs-4 fw-semibold"><?=e($latest ?: 'Unbekannt')?></div>
        ...
        <?php else: ?><div class="alert alert-secondary mt-3 mb-0"><?=current_language()==='en'?'The installation is up to date.':(current_language()==='fr'?'L’installation est à jour.':'Die Installation ist aktuell.')?></div><?php endif; ?>
      <?php endif; ?>
    </div></div>
  </div>
...
<div class="card shadow-sm mt-4" id="migration-backups">
  <div class="card-body p-4">
    <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-3">
      ...
      <form method="post" data-confirm="Jetzt ein vollständiges Migrationsbackup erstellen? Je nach Umfang der Audiodateien kann die ZIP groß werden.">
    ...
    <form method="post" enctype="multipart/form-data" class="row g-2 align-items-end mb-4">
      ...
      <div class="col-md"><label class="form-label" for="migration_backup">Backup von einer anderen Instanz hochladen</label><div class="input-group"><label class="btn btn-outline-secondary mb-0" for="migration_backup"><?=current_language()==='en'?'Choose file':(current_language()==='fr'?'Choisir un fichier':'Datei auswählen')?></label><span class="form-control text-body-secondary text-truncate" data-file-name><?=current_language()==='en'?'No file selected':(current_language()==='fr'?'Aucun fichier sélectionné':'Keine Datei ausgewählt')?></span><input class="visually-hidden" type="file" id="migration_backup" name="migration_backup" accept=".zip,application/zip" required onchange="this.closest('.input-group').querySelector('[data-file-name]').textContent=this.files.length?this.files[0].name:<?=htmlspecialchars(json_encode(current_language()==='en'?'No file selected':(current_language()==='fr'?'Aucun fichier sélectionné':'Keine Datei ausgewählt')),ENT_QUOTES)?>"></div></div>
    ...
    <div class="list-group">
      ...
      <div class="list-group-item d-flex flex-wrap justify-content-between align-items-center gap-2">
        ...
        <div class="d-flex flex-wrap gap-2">
          ...
          <form method="post" data-confirm="Dieses Migrationsbackup endgültig löschen?"><input type="hidden" name="csrf" value="<?=csrf_token()?>"><input type="hidden" name="action" value="delete_migration_backup"><input type="hidden" name="backup" value="<?=e(basename($backup))?>"><button class="btn btn-sm btn-danger d-inline-flex align-items-center justify-content-center" title="Backup löschen" aria-label="Backup löschen" style="width:2rem;height:2rem;padding:0"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="currentColor" viewBox="0 0 16 16" aria-hidden="true"><path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5Zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5Zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6Z"/><path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1 0-2H6a2 2 0 0 1 4 0h3.5a1 1 0 0 1 1 1ZM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118ZM6.5 2h3a1 1 0 0 0-1-1h-1a1 1 0 0 0-1 1Z"/></svg></button></form>
        </div>
      </div>
      <?php endforeach; ?>
