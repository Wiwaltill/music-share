...
$ogTags = '<meta property="og:type" content="music.album">'
    ...
    . ($socialImage !== '' ? '<meta property="og:image" content="'.e($socialImage).'"><meta property="og:image:alt" content="'.e(t('cover.alt_from')).' '.e((string)$share['title']).'">' : '')
...
<?php if($cover):?><div class="modal fade" id="coverModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered modal-xl"><div class="modal-content bg-transparent border-0"><div class="modal-body p-0 text-center position-relative"><button type="button" class="btn-close btn-close-white cover-modal-close" data-bs-dismiss="modal" aria-label="<?=e(t('text.schlieen'))?>"></button><img class="cover-modal-image" src="<?=e($cover)?>" alt="<?=e(t('text.cover.gro'))?>"></div></div></div></div><?php endif?>
<div id="floatingPlayer" class="floating-player" hidden><div class="player-meta"><img src="<?=e($cover)?>" alt=""><div class="min-w-0"><div id="nowPlaying" class="fw-semibold text-truncate"></div><div class="small opacity-75"><?=e($share['artist'])?></div></div></div><audio id="mainPlayer" controls playsinline></audio><button id="closePlayer" class="player-close" aria-label="<?=e(t('text.player.schlieen'))?>">×</button></div><script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script><script src="https://cdn.jsdelivr.net/npm/plyr@3.8.4/dist/plyr.polyfilled.min.js"></script><script>window.MusicShareTranslations=<?=json_encode(js_language_catalogue(),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)?>;window.msT=(key,fallback)=>window.MusicShareTranslations[key]??fallback??key;</script><script>window.MusicShareStats={enabled:<?=statistics_enabled()?'true':'false'?>,endpoint:<?=json_encode(base_url('statistics_event.php'))?>,token:<?=json_encode($isPreview?'':$token)?>,albumId:<?=(int)$share['album_id']?>};</script><script src="<?=base_url('assets/js/player.js?v='.rawurlencode(APP_VERSION))?>"></script><script src="<?=base_url('assets/js/album-colors.js?v='.rawurlencode(APP_VERSION))?>"></script></body></html>
