...
if($isPreview){
    ...
    $s=$pdo->prepare('SELECT id album_id,title,artist,release_year,genre,label_name,copyright_text,description,cover_file FROM albums WHERE id=?');
    ...
    $s=$pdo->prepare('SELECT s.*,a.title,a.artist,a.release_year,a.genre,a.label_name,a.copyright_text,a.description,a.cover_file FROM shares s JOIN albums a ON a.id=s.album_id WHERE s.token=?');
    $s->execute([$token]);
    $share=$s->fetch();
